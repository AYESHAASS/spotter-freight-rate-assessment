...
| `baseline.py` | Model comparison: Linear Regression, Random Forest, and XGBoost on the same holdout |
...
covers November to December 2025, the future relative to training data. A
...
(around 0.03 to 0.04). An ablation using the same model configuration and
holdout as the final pipeline confirmed removing them improves holdout
accuracy:

| Feature set | MAE | RMSE | MAPE |
|---|---|---|---|
| Full (incl. market_index, quote_signal) | $158.33 | $666.92 | 8.80% |
| Structural only (dropped, final choice) | **$153.22** | $666.18 | **7.09%** |

Note: these ablation numbers match the final holdout metrics reported at
the end of this README because both used the identical model
configuration (500 trees, reg_lambda 1.0) and imputation strategy. An
earlier exploratory version of this ablation used a lighter model (400
trees, no explicit regularization) and produced slightly different
absolute numbers (MAE 155.37 vs 147.90); the table above supersedes that
early test and is the one actually used to justify the final feature set.
...
- 183 rows with `posted_rate` >3 std from the mean were flagged but kept
  An unusually high rate isn't necessarily an error (it could be a real
...
$6,921, 3 to 4 times higher than what distance-based pricing predicts (e.g.
...
sub-200-mile hauls to $293 on 2,000-plus mile hauls, expected heteroscedasticity
...
holdout. The single-window MAE of about $153 was somewhat optimistic. Notably,
